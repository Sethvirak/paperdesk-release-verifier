...
def run_bootstrap_self_test(value):
    """Prove the immutable package boots before privileged identities exist.

    This branch executes before activation parsing or construction of any ARM,
    Storage, GitHub, production, or signing client.  It can only attest the
    exact package/site/control bytes and emits one canonical non-secret marker.
    """
    raw=value
    ...
    fields={"schemaVersion","mode","siteName","packageSha256","nonce","issuedAt","expiresAt"}
    if (not isinstance(control,dict) or set(control)!=fields or control.get("schemaVersion")!=1 or control.get("mode")!="package-fetch-self-test"
            ...
            or not core.NONCE.fullmatch(str(control.get("nonce"))) or core.canonical(control).decode()!=raw):core.fail("entry-bootstrap-self-test")
    ...
    marker={"schemaVersion":1,"status":"package-fetch-self-test-complete","siteName":control["siteName"],"packageSha256":control["packageSha256"],"controlSha256":hashlib.sha256(raw.encode()).hexdigest()}
...
def main():
    ...
    activation_doc=load_env_json("PAPERDESK_PRIVATE_RELEASE_ACTIVATION_JSON")
    ...
    activation=core.load_activation_document(activation_doc,runtime_workflow_sha=workflow_sha,observed_bridge_package_sha256=package_sha,provisioning_evidence=provisioning_evidence)
